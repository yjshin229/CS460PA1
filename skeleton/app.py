...
def register_user():
	...
	if test:
		print(cursor.execute("INSERT INTO Users (email, password, firstName, lastName, DateofBirth,hometown, gender) VALUES ('{0}', '{1}')".format(email, password)))
		conn.commit()
		#log user in
		user = User()
		user.id = email
...
def getFriendsList(uid):
    cursor = conn.cursor()
    cursor.execute("SELECT follower_id FROM Friends WHERE user_id ='{0}'".format(uid))
    data = cursor.fetchall()
    ...
    friendsList = [(str(item[0], str(item[1]))) for item in friends]
    return friendsList
...
def userActivity():
    cursor = conn.cursor()
    cursor.execute("SELECT Users.user_id AS uid COUNT(picture_id),COUNT(comments_id) FROM Users,Pictures WHERE uid = Pictures.user_id GROUP BY uid ORDER BY COUNT(picture_id) DESC LIMIT 10")
    data = cursor.fetchall()
